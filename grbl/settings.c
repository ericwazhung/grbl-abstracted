...
uint8_t settings_store_global_setting(uint8_t parameter, float value) {
  ...
  if (parameter >= AXIS_SETTINGS_START_VAL) {
    ...
    uint8_t int_value = g_trunc(value);
    switch(parameter) {
      case 0: 
...
gpioPortWidth_t get_step_pin_mask(uint8_t axis_idx)
{
...
gpioPortWidth_t get_direction_pin_mask(uint8_t axis_idx)
{
...
gpioPortWidth_t get_limit_pin_mask(uint8_t axis_idx)
{
